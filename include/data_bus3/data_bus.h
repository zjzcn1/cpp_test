...
namespace data_bus {
    ...
    public:
        ...
        static void publish(const std::string topic, Ptr<T> data) {
            ...
            if (instance()->publisher_map_.find(topic) == instance()->publisher_map_.end()) {
            ...
            instance()->publisher_map_[topic]->putData(message);
        }
        ...
        static long subscribe(const std::string topic, const std::string subscriber_name, const Callback<T> &callback,
            ...
            Ptr<Publisher> queue_info = instance()->publisher_map_[topic];
        ...
        static bool unsubscribe(long subscriber_id) {
            std::lock_guard<std::mutex> locker(instance()->mutex_);
            for (std::pair<std::string, Ptr<Publisher>> pair : instance()->publisher_map_) {
